...
image: https://www.horizen.io/academy/static/img/p2p-networks/p2p-networks-meta.jpeg
...
The distributed **Peer-to-Peer** (_P2P_) network of a decentralized blockchain is highly resilient because the [network’s nodes](architecture/blockchain-nodes.md) run independently from one another.

We’ll focus on the [protocol of a blockchain](architecture/blockchain-protocols.md) at the _network layer_. At the network layer, peers are identified by their IP addresses.
...
One example of a truly [distributed system](consensus/consensus-in-distributed-systems.md) on the internet is _BitTorrent_. 
...
Many nodes, in the case of the _Horizen_ network, more than _35,000_, store a copy of the blockchain. This is not very storage efficient but makes the blockchain highly resilient against [attacks](security/blockchain-attacks.md) and gives it its immutability.
...
This term sounds very simple at first but there is a bit more to it. The field of graph theory is a mathematical discipline studying the different types of graphs. We touch on graph theory in an article on [Directed Acyclic Graphs](scalability/directed-acyclic-graph-dag.md) or DAGs. 

There, we talked about graphs in the context of [data structures](fundamentals/blockchain-data-structure.md) - namely how blocks can be interconnected at the application layer. Here the graph describes the connectivity between nodes at the network layer.
...
When we talked about [distributed consensus](consensus/consensus-mechanisms.md) we introduced two types of malfunctions at the network layer: **_node failures_** and **_network failures_**.
...
Not only does the [consensus mechanism](consensus/consensus-mechanisms.md) have to account for node failures and network failures, but so does the P2P network itself. 
...
Another good reason to run a node is to safely accept payments. Most [wallets](wallets/crypto-wallets.md) are actually light nodes. They connect to a full node in order to “speak” with the blockchain. 
