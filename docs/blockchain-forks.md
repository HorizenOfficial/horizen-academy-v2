...
One of the main benefits of blockchain technology is that it gives anyone the freedom to opt into their preferred [system of governance](blockchain-governance.md).

In the world of blockchains, systems are created and managed by transparent and [censorship resistant protocols](blockchain-protocols.md), which are rules written into computer code that determine how participants must engage with each other in order to achieve a desired goal.
...
- The process for _mining_ new BTC under a proof of work [consensus mechanism](consensus-mechanisms.md)
...
Depending on the severity of the changes made in the upgrade, miners (those that operate nodes, validate transactions and add new blocks to the chain) can choose or not choose to also upgrade their [nodes software](blockchain-nodes.md) to implement those changes.
...
_Segwit_ introduced several upgrades to the [Bitcoin](bitcoin-glossary.md) blockchain, including fixing the problem of transaction malleability and changing the factor used to determine data storage limits within a single block from size to weight.
...
Those who hold the older chain's [cryptocurrency](cryptocurrency.md) are given a new cryptocurrency from the new chain.
...
The DAO Hack was an incident that occurred on the Ethereum network in 2016 in which $60 million in ETH was stolen from a DAO that raised $150 million when a vulnerability was discovered in its [smart contract](smart-contracts.md).
