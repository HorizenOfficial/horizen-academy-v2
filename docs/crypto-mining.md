...
Mining describes the activity of performing a computationally expensive task - the [Proof of Work (PoW)](proof-of-work-pow.md).
...
So what is the puzzle that miners are trying to solve? This is where we need to talk about [hash functions](hash-functions.md) again.
...
Not only does PoW prevent _Byzantine_ actors from obstructing the block production process, but it also protects the history recorded on the blockchain from being tampered with. All nodes following the [protocol of a blockchain](blockchain-protocols.md) will reject versions of the blockchain that do not have a chain of references form the current block all the way back to the genesis block, each protected with a Proof of Work.
...
In our article on [PoW](proof-of-work-pow.md) we introduced two metrics to assess computational hardness. 
