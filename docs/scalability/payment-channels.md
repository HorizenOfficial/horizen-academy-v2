...
We introduced [sidechains](scalability/sidechains.md) as a scaling approach that spreads the workload otherwise performed by a single set of mainchain nodes to several sets of nodes, each responsible for their sidechain.

We also talked about [Directed Acyclic Graphs (DAGs)](scalability/directed-acyclic-graph-dag.md) that hold the potential to dynamically adjust the on-chain, or "_on-DAG_", throughput by introducing a new type of [data structure](fundamentals/blockchain-data-structure.md) supporting _two-dimensionality_ in an otherwise mostly _one-dimensional_ blockchain world.
...
- **Regular transactions** in the [UTXO model](architecture/utxo-vs-account-model.md)
- **P2SH addresses**, and more specifically [_MultiSig addresses_](wallets/multisig.md)
- [**Cryptographic hash functions**](cryptography/hash-functions.md)
- **Timelocks**
...
Whereas the underlying blockchain derives its security from the computational power of its miners in [Proof of Work](consensus/proof-of-work-pow.md) blockchains, _payment channels_ derive security from economic disincentives. 
...
This can be understood as a simple [smart contract](defi/smart-contracts.md) controlling funds, the **channel balance**, and defining the conditions under which these funds can be spent. 
...
Payment channel networks are built from multiple separate channels that can be coupled when needed. There are several payment channel networks built on top of different [blockchain protocols](architecture/blockchain-protocols.md), some even making protocols interoperable:
...
- [Bolt](https://eprint.iacr.org/2016/701.pdf) is a network proposed by Matthew Green and Ian Miers in 2016. _Bolt_ stands for _Blind Off-chain Lightweight Transactions_. It is currently being considered for the _Zcash_ protocol by the Zcash Foundation, but could also be deployed on top of other blockchain protocols. 
	- Bolt achieves high privacy guarantees by leveraging _blind signatures_ and [_zero-knowledge proofs_](privacy/zero-knowledge-proofs-zkp.md). To make privacy guarantees even stronger, the channel opening can be done through _shielded transactions_.
	-  The authors not only plan to deploy Bolt on Zcash, but also retrofit it to offer a private payment channel option on [Bitcoin](cryptocurrency/bitcoin-glossary.md) and Litecoin.
...
[Blockchain nodes](architecture/blockchain-nodes.md) are agnostic of payment channels and have no way to verify whether a broadcast transaction represents the most recent channel state or an old one.
...
Note: For the sake of simplicity we consider [private keys](wallets/wallet-addresses.md) and [digital signatures](cryptography/digital-signatures.md) as "equivalent" in this article. 
...
As the name suggests, HTLCs rely on cryptographic [hash functions](cryptography/hash-functions.md) and for the most part on two of their fundamental properties: 
...
With regards to throughput, it suffices to look at the events around the launch of [CryptoKitties](https://www.cryptokitties.co/) on the [Ethereum](cryptocurrency/ethereum-glossary.md) network in November 2017.
...
Many dApps don't just rely on the transfer of a [token](tokenomics/what-is-a-token.md), but also the transmission of data. This data, _sent to a smart contract_, updates the contracts state to represent the most recent contract interactions. The idea behind more generalized state channel constructions is moving these data transfers off-chain into state channel networks where they can be performed at a lower financial and computational cost. 
