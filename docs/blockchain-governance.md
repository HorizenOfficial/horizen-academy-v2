...
One of a [blockchain protocol's](blockchain-protocols.md) most highly valued properties is its level of decentralization. There is usually intense scrutiny of the process for generating new blocks, but less focus on the process for changing a protocol's code. 
...
The **Minimum Nakamoto Coefficient** is determined by assigning a decentralization score to the different subsystems of a blockchain: 
 ...
 - Exchanges
 - Nodes
 - And [wallet distribution](crypto-wallets.md)
...
Realistically there cannot always be a process to choose the deciders. Most projects in blockchain start with a few like-minded people with an idea of what their ideal [_cryptocurrency_](cryptocurrency.md), _dApp_ or _blockchain_ platform should look like. 
...
An important design decision is the choice of the [consensus mechanism](consensus-mechanisms.md), and in case of [Proof of Work](proof-of-work-pow.md) blockchains, the [mining algorithm](crypto-mining.md). 
...
If one had to find the closest thing to a formal specification for [Bitcoin](bitcoin-glossary.md), it would be the code of the Bitcoin Core repository.
...
If we were to apply Schneider's logic to this process, this would mean that "political" suggestions from developers are decided upon by the "fiscal power" of miners. Once a decision is made, the [nodes](blockchain-nodes.md) on the network enforce the new rules in their role as the administrators. 
...
The [Decentralized Autonomous Organization](decentralized-autonomous-organization-dao.md) (DAO) is an entity governed by rules written into the code. It acts predictably according to a predefined "constitution" and is controlled by its shareholders. 
...
Interactions with a DAO are tracked with timestamps and [digital signatures](digital-signatures.md) recorded on-chain. The general concept was introduced as a [_decentralized autonomous corporation_](https://letstalkbitcoin.com/is-bitcoin-overpaying-for-false-security) (DAC) in an article by Dan Larimer in 2013. Vitalik Buterin went one step further and proposed that human management could be made obsolete if the [smart contracts](smart-contracts.md) governing the DAO were written in a _turing complete_ language.
...
About **$50** million in crypto assets were stolen by the hackers. The hack was reverted by reorganizing the [Ethereum blockchain](ethereum-glossary.md), which led to a chain split. The **Ethereum Classic** chain is the Ethereum chain where the hack was not reverted, while investors on the Ethereum chain were refunded their assets.
...
Miners considered the threat to the network's utility, and hence value, serious enough to follow the [majority of nodes](blockchain-nodes.md) and activate SegWit. An important learning at that time was that the miners influence on the network can be overruled by its users when users form a strong consensus.
...
When a sufficiently large share of hash power has signaled readiness for a [fork](blockchain-forks.md), full nodes will enforce those rule changes accordingly.
